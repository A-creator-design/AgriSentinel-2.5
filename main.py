...
OS_KEY = "AIzaSyCZa0PUfx4Xm_h1QhjFpC4QBsnoQaI0_BU"
    ...
    with col2:
        if st.button("🔍 ANALYZE & SPEAK"):
            try:
                ...
                # --- VOICE ENGINE (Cleaned for No Asterisks) ---
                # This line removes all stars so the voice doesn't say "Asterisk"
                clean_txt = txt.replace("*", "") 
                
                sound_io = BytesIO()
                # Use clean_txt instead of txt
                tts = gTTS(text=clean_txt, lang=lang_codes[lang_opt]) 
                tts.write_to_fp(sound_io)
                sound_io.seek(0)
                
                st.audio(sound_io, format="audio/mp3", autoplay=True)
                st.success("🔊 Reading clean report aloud...")
